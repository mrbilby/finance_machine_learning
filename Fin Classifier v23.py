...
def compile_data():
    ...
    for ticker in tickers:
        ...
        if count % 10 == 0:
            print(count)
    print(main_df.head())
    main_df.to_csv('fin_joined_google_closes.csv')

def stock_remapper():
    df=pd.read_csv('fin_joined_google_closes.csv')
    df = df.drop('Date',1)
...
##            print(ticker+" Sell")
        ...
        y=y+1
    origdf=pd.read_csv('fin_joined_google_closes.csv')
